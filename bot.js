...
import { triviaStateAndQuestion, triviaStateManager } from './functions/triviaHandler.js';
...
// Load the mode from .env (default to 'live' if missing)
...
const CONFIG = {
    ...
    custom: {
        apiBase: `http://${process.env.CUSTOM_HOST}:33000/mock/helix`,
        authBase: `http://${process.env.CUSTOM_HOST}:33000/mock/oauth2`,
        wsUrl: `ws://${process.env.CUSTOM_HOST}:33001`
    }
};
...
console.log(`Connecting to: ${activeConfig.wsUrl}`);



// Example: WebSocket Connection
const client = new WebSocket(activeConfig.wsUrl);


const EVENTSUB_WEBSOCKET_URL = activeConfig.wsUrl;
...
async function getAuth() {
    const response = await fetch(`${activeConfig.authBase}/validate`, {
        method: 'GET',
        headers: {
...
async function handleWebSocketMessage(data) {
    switch (data.metadata.message_type) {
        ...
        case 'notification': {
            ...
            let { text: fullText } = data.payload.event.message;
            ...
            let isPrefixed = messageLower.startsWith(BOT_KEYWORD);

            // if (triviaStateAndQuestion.triviaOn && chatterName != process.env.BOT_USERNAME) {
            //     //here
            //     fullText = `${BOT_KEYWORD} check trivia ${fullText}`;
            //     isPrefixed = true;
            // }

            if (triviaStateAndQuestion.triviaOn && chatterName != process.env.BOT_USERNAME && !isPrefixed) {
                fullText = `${BOT_KEYWORD} check trivia ${fullText}`;
                isPrefixed = true;
            }
                ...
                if (messageAfterPrefix.startsWith("ask")) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            queueChatMessage(`@${chatterName}, an Error happened, please try again.`)
...
async function sendChatMessage(chatMessage) {
    ...
    const response = await fetch(`${activeConfig.apiBase}/chat/messages`, {
        method: 'POST',
        headers: {
...
async function registerEventSubListeners() {
    const response = await fetch(`${activeConfig.apiBase}/eventsub/subscriptions`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${await getAccess_token()}`,
            'Client-Id': CLIENT_ID,
...
async function isChannelLive(username) {
    const response = await fetch(`${activeConfig.apiBase}/streams?user_id=${username}`, {
        headers: {
            'Authorization': `Bearer ${await getAccess_token()}`,
            'Client-Id': CLIENT_ID
        }
